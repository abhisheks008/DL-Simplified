...
import torch
import numpy as np
# from timm.models.WSConv_utils import WSLinear, WSConv2d, WSConv2d_v1, GroupLinear
...
def WSlinear_flops_counter_hook(module, input, output):
    input = input[0]
    # pytorch checks dimensions, so here we don't care much
    output_last_dim = output.shape[-1]
    bias_flops = output_last_dim if module.bias is not None else 0
    module.__flops__ += int(np.prod(input.shape) * output_last_dim / np.prod(module.multiplier) + bias_flops)
def Grouplinear_flops_counter_hook(module, input, output):
    input = input[0]
    # pytorch checks dimensions, so here we don't care much
    output_last_dim = output.shape[-1]
    bias_flops = output_last_dim if module.bias is not None else 0
    module.__flops__ += int(np.prod(input.shape) * output_last_dim / np.prod(module.groups) + bias_flops)
...
MODULES_MAPPING = {
    ...
    # WS ops
    # WSLinear: WSlinear_flops_counter_hook,
    # GroupLinear: Grouplinear_flops_counter_hook,
    # Upscale
